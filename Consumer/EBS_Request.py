...
class BaseEBSAPIView(views.APIView):
    ...
    verify_ssl = True
    validated_data = None  # READ ONLY. DO NOT CHANGE ITS CONTENT
    ...
    def handle_ebs_response(self, ebs_response):
        if ebs_response.status_code == 200:
            ...
            if request and hasattr(request, 'body') and request.body:
                ...
                request_data = None
            logger.error(
                "Failed to process EBS request. url: %s, response code: %s, response data: %s, request: %s",
    ...
    def is_ebs_200_response_successful(self, ebs_response_json, raise_exception=False):
        if str(ebs_response_json['responseCode']) != '0':
            ...
            if hasattr(self, 'request') and hasattr(self.request, 'data')and self.request.data:
                ...
                request_data = None
            logger.error(
                "EBS request is rejected. url: %s, response code: %s, response data: %s, request: %s",
                url, status_code, response_data, request_data)
            ...
            logger.info("EBS request is successful. url: %s, response code: %s, response data: %s, request: %s",
               url, status_code, response_data, request_data)
            return True
    ...
    def ebs_post(self, payload):
        ...
        logger = self.get_logger()
        logger.info('Our request to EBS is {}. EBS response is {}'.format(payload, ebs_response.json()))
        return ebs_response
...
# class EBSMerchantAPIView(BaseEBSAPIView):
#     ebs_base_url = settings.EBS_MERCHANT_API["END_POINT"]
#     verify_ssl = settings.EBS_MERCHANT_API["VERIFY_SSL"]
#     timeout = settings.EBS_MERCHANT_API["TIMEOUT"]
#     client_id = settings.EBS_MERCHANT_API["CLIENT_ID"]

#     def get_payload_from_input(self, input_data):
#         payload = {}
#         payload.update(input_data)
#         payload.update({'clientId': self.client_id})
#         return payload

#     def get_response_data(self, ebs_response_content_json):
#         ebs_response_content_json.pop('clientId')
#         return ebs_response_content_json
